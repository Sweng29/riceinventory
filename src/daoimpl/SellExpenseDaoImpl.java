 ...
 */
public class SellExpenseDaoImpl implements SellExpenseDao{
    
    public int insertPurchaseExpense(PurchaseExpenseModel purchaseExpenseModel) {
        try{
            PreparedStatement ps = DBConnection.getInstance().prepareStatement("insert into purchase_sell_expense(sell_id,bank_id,payment_type,transport_expense,labour_expense,other,broker_comission,broker_paid_amount,comments,created_by,created_date)  values(?,?,?,?,?,?,?,?,?,?,now())");
            ps.setInt(1, purchaseExpenseModel.getSellModel().getSellId());
    ...
    public ResultSet getAllPurchaseExpense(SellModel sellModel) {
        try{
            PreparedStatement ps = DBConnection.getInstance().prepareStatement("SELECT p.expense_id, p.sell_id AS 'Sell No',IFNULL(b.acc_no,'-') AS 'Acc No',p.payment_type AS 'Payment Type',p.`transport_expense` AS 'Transport',p.`labour_expense` AS 'Labour',p.`other` AS 'Other',p.`broker_comission` AS 'Broker Comm',p.`broker_paid_amount` AS 'Broker Paid',p.`comments` AS 'Comment' FROM `purchase_sell_expense` p LEFT JOIN bank b ON p.`bank_id` = b.`bank_id` WHERE p.`sell_id` = ? AND p.active = 1");
            ps.setInt(1, sellModel.getSellId());
    ...
    public int insertPurchaseExpenseWithoutBank(PurchaseExpenseModel purchaseExpenseModel) {
     try{
            PreparedStatement ps = DBConnection.getInstance().prepareStatement("insert into purchase_sell_expense(sell_id,payment_type,transport_expense,labour_expense,other,broker_comission,broker_paid_amount,comments,created_by,created_date) values(?,?,?,?,?,?,?,?,?,now())");
            ps.setInt(1, purchaseExpenseModel.getSellModel().getSellId());
